COS20031 - Swinburne University of Technology


<!-------How to run our website---------!>
<h1> Hey there! We are TANK 👋 </h1>
<h2> A Passionate, Hard working and Dedication TEAM From Vietnam vn </h2>
<img align="right" alt="GIF" src="https://camo.githubusercontent." width="500"/>
<h3> 👨🏻‍💻 Steps to run and test our website </h3>

- 🔭 &nbsp; STEP 1: Git clone/Download the source code folder to your own computer
- 🤔 &nbsp; STEP 2: Place it into the folder htdocs in XAMPP folder like this path "\XAMPP\htdocs"
- 🎓 &nbsp; STEP 3: Open a new tab in your web browser, type "localhost\(your path of the folder code you place it)"
- 💼 &nbsp; STEP 4: Enjoy using our website, purchasing courses and applying jobs you want =)))).






